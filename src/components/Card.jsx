...
const Card = ({ card, isTop, onClick, shakeCardId }) => {
  ...
  return (
    <div
      ref={isTop ? drag : null}
      onClick={handleClick}
      className={`w-8 h-12 sm:w-16 md:w-20 sm:h-24 md:h-28 rounded shadow-md mb-[-40px] sm:mb-[-60px] z-10 relative border-1 sm:border-2 ${border} ${
        card.faceUp ? `${bg} text-black` : 'bg-gray-700'
      } ${isDragging ? 'opacity-50' : ''} cursor-pointer ${shakeCardId === card.id ? 'animate-shake' : ''}`}
    >
      {card.faceUp ? (
        <div className="w-full h-full p-[1px] sm:p-1 relative flex flex-col justify-between">
          {/* Top-left rank */}
          <div className="absolute top-0 left-0 text-[9px] sm:text-xs md:text-base font-bold">
            {card.rank}
          </div>
          ...
          <div className="flex-grow flex items-center justify-center">
            <img
              src={iconSrc}
              alt={card.class}
              className="w-5 h-5 sm:w-8 sm:h-8 md:w-13 md:h-13 object-contain"
              loading="lazy"
            />
          </div>

          {/* Bottom-right rank (mirrored) */}
          <div className="absolute bottom-0 right-0 text-[9px] sm:text-xs md:text-base font-bold transform rotate-180">
            {card.rank}
          </div>
        </div>
      ) : (
